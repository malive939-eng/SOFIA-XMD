...
const port = por.PORT;
const ownerNumber = ["94717775628","94779702531"];
if (!app) return;
// Session directory
...
async function sessdl() {
  try {
    ...
    const base64Data = sess.SESSION_ID.split("SOFIA-XMD~")[1];
    if (!base64Data) {
...
async function connectToWA() {
  ...
  //===========================
  console.log("🔥 SOFIA-XMD is starting...");
  ...
  conn.ev.on("connection.update", (update) => {
    ...
    if (connection === "close") {
      ...
      let up = `
          🚀 **© 𝐒𝐎𝐅𝐈𝐀-𝐗𝐌𝐃 💚 Connected Successfully!** ✅ 
          
          --- **🎉 Welcome to © 𝐒𝐎𝐅𝐈𝐀-𝐗𝐌𝐃 💚!** 🎉 
      ...
      conn.sendMessage(conn.user.id, {
        text: up,
        contextInfo: {
          mentionedJid: ["94740237746@s.whatsapp.net"], // specify mentioned JID(s) if any
          groupMentions: [],
          forwardingScore: 999,
          isForwarded: true,
...
if (!app) return;
app.get("/", (req, res) => {
  res.send("hey I am alive, SOFIA-XMD Is started✅");
});
app.listen(port, () =>
